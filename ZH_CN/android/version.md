...
- 内部优化
	...
	- **解决游戏客户端微信Key的问题**，游戏初始化时初始化微信appkey改为初始化msdkkey。**`具体的msdkkey届时可以在游戏注册平台查询（查看手Q、微信APPID的地方），切记游戏不要直接将微信AppKey赋值给MSDKkey`**
	- MSDK的本地数据信息加密保存
...
- SDK升级
	- 升级opensdk，解决x86平台上opensdk页面登录crash问题
