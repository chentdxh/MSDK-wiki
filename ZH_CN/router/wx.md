...
| json|json对象|战斗json数据 |
	...
	{
		"baseinfo": {
				"gamename": "全民飞机大战", //字符串型，游戏名称
				"platid": 0, //整型，平台类型 0：iOS 1：Andriod
				"partitionid": "9", //字符串型，分区ID
				"roleid": "wongcai", //字符串型，角色ID
				"level": 2 //整型，用户等级，游戏没有等级概念可省略
				},
		"battleinfo": {
				"score": 4288625, //整型，每局（关）分数，游戏没有分数概念可省略
				"iswin": true //布尔型，当前局是否胜利，适合棋牌类，没有胜局概念可省略
				},
		"userdefined": { //游戏自定义数据，比如某游戏有金币和战力两种数据：
				"goldcoin": { //自定义数据ID1
						"name": "金币", //字符串型，数据名称
						"value": 358267, //整型，数据数值
						"isrank": true //布尔型，微信游戏排行榜仅会选取一个数据做为排序指标，假如指定了这个数据(未指定则默认使用battleinfo的分数score)，那么其它数据就不能设置isrank=true
						},
				"power": { //自定义数据ID2
						"name": "体力", //字符串型，数据名称
						"value": 86542, //整型，数据数值
